...
class SegmentationPairwiseMetric(ABC):
    ...
    def _check_header(s: pd.DataFrame):
        if s.columns[:3].tolist() != ["chr", "start", "end"]:
            raise ValueError(
                "Segmentation comparison is available only between dataframes containing 'chr', 'start' and 'end' columns"
            )
    ...
    def compare(
        self, s1: pd.DataFrame, s2: pd.DataFrame, by_chrom=False, estimator=None
    ):
        self._check_header(s1)
        self._check_header(s2)
        result = {}
        for chrom in set(s1.chr).intersection(set(s2.chr)):
            chrom_metric = self.compare_segments(
                s1[s1.chr == chrom], s2[s2.chr == chrom]
            )
...
def moc(cds_p, cds_q):
    ...
    def __aggregate_overlaps(cds_p, cds_q):
        ...
        for cd_i in cds_p:
            for cd_j in cds_q:
                r += (__get_overlap(cd_i, cd_j) ** 2) / (
                    __get_size(cd_i) * __get_size(cd_j)
                )
        return r

    cds_p, cds_q = __make_same_limits(
        cds_p + __fill_gaps(cds_p), cds_q + __fill_gaps(cds_q)
    )
    moc = None
    Np = len(cds_p)
    Nq = len(cds_q)
