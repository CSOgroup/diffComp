...
class CalderDifferentialCompartments:
    CALDER_DIFFERENTIAL_COMPARTMENTS_HEADER = ['chr', 'start', 'end', 'value', "rank1", "comp1", "rank2", "comp2"]
    ...
    def __init__(self,
        ...
        self._segs['chr'] = pd.Categorical(self._segs['chr'], categories=chroms, ordered=True)
    ...
    def concat(segmentations: list[CalderDifferentialCompartments], **kwargs) -> CalderDifferentialCompartments:
        ...
        return CalderDifferentialCompartments(input_df=all_segs, signal_df = all_signals, expected_std=all_expected_std, **kwargs)
...
class CalderDifferentialSegmentator(ABC):
    ...
    def segment(self, s1: CalderSubCompartments, s2: CalderSubCompartments,
                ...
                chroms: Optional[List[str]] = None) -> CalderDifferentialCompartments:
        _logger = logging.getLogger(self.__class__.__name__)
        ...
        if chroms is None:
            ...
            if len(set(chroms).difference(shared_chroms)) != 0:
                raise ValueError(f"Specified chromosomes are not present in the datasets")
...
class CalderRecursiveDifferentialSegmentator(CalderDifferentialSegmentator):
    ...
    def get_pvalue(self, core: dict) -> Tuple[float, float]:
        ...
        def __gamma_pvalue(core):
            ...
            pvalue = ((self._null_dist["gamma"][chrom] > statistic).sum() + 1)/(self._null_dist["gamma"][chrom].shape[0] + 1)
        ...
        def __delta_pvalue(core):
            ...
            if (~np.isnan(v)).sum() > 0:
                ...
                pvalue = ((self._null_dist["delta"][chrom] > statistic).sum() + 1)/(self._null_dist["delta"][chrom].shape[0] + 1)
            else:
                statistic = np.NaN
                pvalue = np.NaN
            return statistic, pvalue
    ...
    def segment_chromosome(self,
        ...
        s12_rank = self.get_binned_deltaRank(s1, s2).sort_values(['chr', 'start', 'end']).reset_index(drop=True)
        ...
        def __recursive_segmentation(X, sub_mean=True):
            ...
            if (std_signal >= exp_std) or (np.isnan(v).sum() > 0):
                ...
                stat_result = self.get_pvalue(core = {
